...
trait VerifyLink
{
    ...
    private function verifyPasswordResetLinkCore($guard = 'web' , $dbName = 'User')
    {
        ...
        $existField = config('laraAuth.auth.validation.register.existsField.'.$guard);
        ...
        if ($this->checkResetPasswordToken($guard))
        {
    ...
    private function checkResetPasswordToken($guard)
    {
        ...
        $dbToken = DB::table('password_resets')->where('email',request()
            ->get(config('laraAuth.email.resetPassword.actionLinkParams.'.$guard)['email']))->first()->token;
